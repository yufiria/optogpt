...
本文件是OptoGPT逆向设计任务的入口脚本，用于从光谱预测多层薄膜结构。
主要功能：
- 解析命令行参数
- 加载和准备数据集
- 构建Transformer逆向设计模型
...
使用方法：
    python run_optogpt.py --epochs 1000 --batch_size 1000 --spec_type R_T
"""
...
if __name__ == '__main__':
    ...
    # 损失函数参数
    parser.add_argument('--smoothing', default=0.1, type=float, help='KL散度的平滑系数')
    ...
    # 模型架构参数
    parser.add_argument('--layers', default=1, type=int, help='编码器层数')
    ...
    parser.add_argument('--save_name', default='model_inverse', type=str, help='模型保存名称')
    ...
    # 构建保存文件名，包含关键超参数
    temp = [args.ratios, args.smoothing, args.batch_size, args.max_lr, args.warm_steps, 
    ...
    data = PrepareData(TRAIN_FILE, TRAIN_SPEC_FILE, args.ratios, DEV_FILE, DEV_SPEC_FILE, 
                      args.batch_size, args.spec_type, 'Inverse')

    # 获取词汇表大小和光谱维度
    tgt_vocab = len(data.struc_word_dict)  # 目标词汇表（结构）
    src_vocab = len(data.dev_spec[0])      # 源维度（光谱）
    ...
    # 构建逆向设计模型
    model = make_model_I(
                    args.spec_dim,     # 源：光谱维度
                    args.struc_dim,    # 目标：结构词汇表大小
                    args.layers, 
                    args.d_model, 
                    args.d_ff,
                    args.head_num,
